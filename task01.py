...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    consts = PrinterConstraints(**constraints)
    ...
    for job in jobs:
        if (len(current_group) < consts.max_items and 
            current_volume + job.volume <= consts.max_volume):
            current_group.append(job)
            current_volume += job.volume
            current_max_time = max(current_max_time, job.print_time)
        else:
            if current_group:
                print_order.extend([g_job.id for g_job in current_group])
